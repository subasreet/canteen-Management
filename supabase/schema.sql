-- ==============================================================================
-- Supabase PostgreSQL Schema & Migration for College Canteen Management System
-- Source of Truth: SRS.md (Functional Requirements FR-01 through FR-06)
-- ==============================================================================

-- 1. Users Table (FR-01: User & Admin Authentication)
...
-- 2. Food Items Table (FR-02: View Menu, FR-06: Admin Add/Update/Remove)
CREATE TABLE IF NOT EXISTS food_items (
    id SERIAL PRIMARY KEY,
    name VARCHAR(150) UNIQUE NOT NULL,
    category VARCHAR(50) NOT NULL CHECK (category IN ('breakfast', 'lunch', 'snacks', 'drinks')),
...
-- 3. Orders Table (FR-04: Place/Confirm Orders, FR-05: Order Status & History)
...
-- 4. Order Items Table (FR-03: Add Items to Order, FR-04: Line Items Breakdown)
...
-- Indexes for performance and high-speed queries (NFR-01, NFR-02)
...
-- Permissive RLS Policies for Supabase
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow public read of food items') THEN
        CREATE POLICY "Allow public read of food items" ON food_items FOR SELECT USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow authenticated admin food modifications') THEN
        CREATE POLICY "Allow authenticated admin food modifications" ON food_items FOR ALL USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow user orders read') THEN
        CREATE POLICY "Allow user orders read" ON orders FOR SELECT USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow user orders insert') THEN
        CREATE POLICY "Allow user orders insert" ON orders FOR INSERT WITH CHECK (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow user order_items read') THEN
        CREATE POLICY "Allow user order_items read" ON order_items FOR SELECT USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow user order_items insert') THEN
        CREATE POLICY "Allow user order_items insert" ON order_items FOR INSERT WITH CHECK (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow user read') THEN
        CREATE POLICY "Allow user read" ON users FOR SELECT USING (true);
    END IF;
END $$;

-- ==============================================================================
-- Initial Seed Data: Existing Records from Canteen System (Without Data Loss)
-- ==============================================================================

-- 1. Users: Canteen Administrator and Demo Student (FR-01)
...
-- 2. Food Items: All 12 Original Menu Items (FR-02)
